...
void Analytic_createParabola(Analytic_Parabola parabola, SDL_Renderer *renderer) {
    ...
    switch (parabola.type) {
        case Analytic_ParabolaY:
            for (float x = (SDL_WINDOW_SIZE / 2) * -1; Analytic_convertXAbsolute(x) <= SDL_WINDOW_SIZE; x += 0.1) {
                float x1 = x + 0.1, y = Analytic_ParabolaYFunction(parabola, x), y1 = Analytic_ParabolaYFunction(
                        parabola, x1);
                if (Analytic_isXRelativeOK(x1) && Analytic_isYRelativeOK(y) && Analytic_isYRelativeOK(y1))
                    SDL_RenderDrawLine(renderer, Analytic_convertXAbsolute(x * DIVISOR / 10),
                                       Analytic_convertYAbsolute(y * DIVISOR / 10),
                                       Analytic_convertXAbsolute(x1 * DIVISOR / 10),
                                       Analytic_convertYAbsolute(y1 * DIVISOR / 10));
            }
            break;
        case Analytic_ParabolaX:
            parabola.a *= -1;
            for (float y = (SDL_WINDOW_SIZE / 2) * -1; Analytic_convertXAbsolute(y) <= SDL_WINDOW_SIZE; y += 0.1) {
                float y1 = y + 0.1, x = Analytic_ParabolaXFunction(parabola, y), x1 = Analytic_ParabolaXFunction(
                        parabola, y1);
    ...
    switch (hyperbole.type) {
        case Analytic_HyperboleY:
            for (float x0 = ((SDL_WINDOW_SIZE / 2) * -1); round(x0) != 0; x0 += 0.1) {
                float x1 = x0 + 0.1, y0 = Analytic_HyperboleYFunction(hyperbole, x0), y1 = Analytic_HyperboleYFunction(
                        hyperbole, x1);
                if (isnan(y1))
                    y1 = 0;
                if (Analytic_isXRelativeOK(x0) && Analytic_isXRelativeOK(x1) && Analytic_isYRelativeOK(y0) &&
                    Analytic_isYRelativeOK(y1)) {
                    SDL_RenderDrawLine(renderer, Analytic_convertXAbsolute(x0 * DIVISOR / 10),
                                       Analytic_convertYAbsolute(y0 * DIVISOR / 10),
                                       Analytic_convertXAbsolute(x1 * DIVISOR / 10),
                                       Analytic_convertYAbsolute(y1 * DIVISOR / 10));
                    SDL_RenderDrawLine(renderer, Analytic_convertXAbsolute(-x0 * DIVISOR / 10),
                                       Analytic_convertYAbsolute(y0 * DIVISOR / 10),
        ...
        case Analytic_HyperboleX:
            for (float x0 = ((SDL_WINDOW_SIZE / 2) * -1); floor(x0) != 0; x0 += 0.1) {
                float x1 = x0 + 0.1, y0 = Analytic_HyperboleXFunction(hyperbole, x0), y1 = Analytic_HyperboleXFunction(
                        hyperbole, x1);
                if (isnan(y1))
                    y1 = 0;
                if (Analytic_isXRelativeOK(x0) && Analytic_isXRelativeOK(x1) && Analytic_isYRelativeOK(y0) &&
        ...
        case Analytic_HyperboleK:
            for (float x0 = ((SDL_WINDOW_SIZE / 2) * -1); Analytic_convertXAbsolute(x0) <= SDL_WINDOW_SIZE; x0 += 0.1) {
                if ((float) (roundf(x0 * 10) / 10) == (float) -0.1) // PREVENT NAN RESULT
                    continue;
                float x1 = x0 + 0.1, y0 = Analytic_HyperboleKFunction(hyperbole, x0), y1 = Analytic_HyperboleKFunction(
                        hyperbole, x1);
                if (Analytic_isXRelativeOK(x0) && Analytic_isXRelativeOK(x1) && Analytic_isYRelativeOK(y0) &&
                    Analytic_isYRelativeOK(y1)) {
                    SDL_RenderDrawLine(renderer, Analytic_convertXAbsolute(x0 * DIVISOR / 10),
        ...
        case Analytic_HyperboleHomographic: {
            ...
            SDL_RenderDrawLine(renderer, Analytic_convertXAbsolute(firstAxe * DIVISOR / 10), SDL_WINDOW_SIZE,
                               Analytic_convertXAbsolute(firstAxe * DIVISOR / 10), 0);
            SDL_RenderDrawLine(renderer, 0, Analytic_convertYAbsolute(secondAxe * DIVISOR / 10), SDL_WINDOW_SIZE,
                               Analytic_convertYAbsolute(secondAxe * DIVISOR / 10));
            SDL_SetRenderDrawColor(renderer, yellow.r, yellow.g, yellow.b, yellow.a);
            for (float x0 = ((SDL_WINDOW_SIZE / 2) * -1); Analytic_convertXAbsolute(x0) <= SDL_WINDOW_SIZE; x0 += 0.1) {
                float x1 = x0 + 0.1, y0 = Analytic_HyperboleHomographicFunction(hyperbole,
                                                                                x0), y1 = Analytic_HyperboleHomographicFunction(
                        hyperbole, x1);
